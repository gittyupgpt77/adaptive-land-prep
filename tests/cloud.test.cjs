...
function harness(){
 let owner='A',data={trainingLogs:'[]'},confirmed=true,error=null,afterInsert=()=>{},insertError=null;
 ...
 const core=createCloudBackup({client,collect:()=>({...data}),validate:p=>{if(!p.data||p.formatVersion!==3)throw Error('invalid')},restore:async f=>restores.push(JSON.parse(await f.text())),confirm:()=>confirmed,assertCurrent:id=>{if(id!==owner)throw Error('account changed')}});
 return {core,writes,restores,filters,payload,setOwner:v=>owner=v,setData:v=>data=v,cancel:()=>confirmed=false,fail:()=>error=Error('network failed'),failInsert:()=>insertError=Error("safety upload failed"),afterInsert:fn=>afterInsert=fn};
}

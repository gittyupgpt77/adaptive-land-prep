...
3. Open the **existing Home Screen app**. Go to Program → View Recovery & Weight Trends → Private cloud backup → **Set Up New Backup**. This switches connection within the same installed app and storage origin. Create an account and confirm its email using Firebase's hosted confirmation page, then return to the Home Screen app and sign in again. The existing Supabase account is separate. The direct `https://gittyupgpt77.github.io/adaptive-land-prep/?backup=firebase` URL is also available for a clean replacement browser, but opening it in a separate Safari tab does not migrate records from the installed app.

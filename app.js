...
function genericMovementVisual(name){const x=name.toLowerCase();let pose="stand";if(x.includes("plank")||x.includes("dead bug"))pose="plank";else if(x.includes("squat")||x.includes("lunge")||x.includes("step"))pose="squat";else if(x.includes("deadlift")||x.includes("hinge")||x.includes("romanian"))pose="hinge";else if(x.includes("pull-up")||x.includes("pull up")||x.includes("hang"))pose="pullup";else if(x.includes("press"))pose="press";else if(x.includes("carry")||x.includes("walk")||x.includes("run"))pose="carry";else if(x.includes("calf")||x.includes("soleus")||x.includes("tibialis"))pose="calf";else if(x.includes("row"))pose="rowpose";else if(x.includes("neck"))pose="neckpose";else if(x.includes("grip")||x.includes("forearm"))pose="grippose";return "<div class='movement-visual'><div class='ground'></div><div class='stick "+pose+"'><i class='head'></i><i class='body'></i><i class='arm a1'></i><i class='arm a2'></i><i class='leg l1'></i><i class='leg l2'></i><i class='bar'></i></div></div>"}
function bindExerciseButtons(root=document){root.querySelectorAll(".exercise-button").forEach(btn=>btn.onclick=e=>{if(e.target.closest(".info-btn"))return;const name=decodeURIComponent(btn.dataset.exercise),visual=movementVisuals[name]||genericMovementVisual(name);openInfo(name,"Review the set and repetition target shown in the workout. Move slowly through setup and technique before adding load.",visual)});bindInfoButtons(root)}

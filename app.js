...
const blocks=[["Re-entry",1,4,"Run/walk 0–2 exposures; C2 2–3×40–60 min","None","Recondition and establish baselines"],["Foundation I",5,8,"2 runs/week; ~3–6 mi","2–4 mi easy","Continuous running tolerance"],["Foundation II",9,12,"2–3 runs/week; ~6–10 mi","3–5 mi easy","Aerobic base and calisthenics"],["Build I",13,16,"3 runs/week; ~10–16 mi","4–6 mi","Controlled threshold introduction"],["Build II",17,20,"3 runs/week; ~14–20 mi","5–8 mi","Long-run growth"],["Build III",21,24,"3 runs/week; ~16–22 mi","6–10 mi","Consolidate and test"],["Specificity I",25,28,"3–4 runs/week; ~20–26 mi","8–10 mi","Running economy and ruck technique"],["Specificity II",29,32,"3–4 runs/week; ~22–30 mi","8–12 mi","Threshold + long aerobic"],["Specificity III",33,36,"4 runs/week; ~26–34 mi","10–14 mi","Consecutive-day exposure"],["Specificity IV",37,40,"3–4 runs/week; ~24–32 mi","10–14 mi","Absorb training"],["Work Capacity I",41,44,"4 runs/week; ~28–38 mi","12–16 mi","Accumulated workload"],["Work Capacity II",45,48,"4–5 runs/week; ~34–45 mi","14–20 mi","Peak specificity"],["Peak / Consolidate",49,52,"~38–50 mi only if earned","16–24 mi only if earned","Peak work-capacity assessment"],["Taper",53,56,"Reduce 30→80%","Reduce then eliminate","Dissipate fatigue"]];
const templates={
...
Fo:["Recovery & mobility","Full-body strength + calisthenics","Easy aerobic row","Easy run + lower-leg durability","Full-body strength — Session B","Long easy run or light ruck","Easy recovery row"],
Bu:["Recovery & mobility","Faster aerobic work + strength","Easy run","Rowing or controlled threshold work","Strength + calisthenics","Long run or ruck","Easy recovery row"],
Sp:["Recovery & mobility","Quality run + calisthenics","Easy run","Strength + easy row","Easy run + loaded carries","Long ruck or run","Recovery aerobic work"],
...
"Full-body strength — Session A":{
 ...
 why:"Build the basic strength that supports running, rucking, calisthenics, posture and injury resistance.",
 steps:[
...
"Full-body strength + calisthenics":{
 title:"Full-body strength + calisthenics",
...
"Long easy run or light ruck":{
 ...
 steps:[
  ...
  ["If rucking","Use weekly ruck target","Continuous walking","Light pack in early phases. Maintain normal gait and upright posture."]
 ]},
"Long run or ruck":{
 title:"Long run or ruck",
...
"Quality run + calisthenics":{
 title:"Quality run + calisthenics",
...
"Strength + calisthenics":{title:"Strength + calisthenics",why:"Maintain strength and improve strict bodyweight performance.",steps:[["Strength","Session A/B","—","Alternate."],["Push-ups & pull-ups","4–6 submaximal sets","—","Avoid daily failure."]]},
...
function evaluate(){const a=systemic(),b=mechanical(),c=fueling(),o=overall(a,b,c);color($("sysStatus"),a);color($("mechStatus"),b);color($("fuelStatus"),c);color($("overallStatus"),o);$("statusPill").textContent=o||"—";$("statusPill").className="status-pill "+(o?o.toLowerCase():"neutral");let run,ruck,c2,strength,intensity,nutrition,warn="";if(!o)run=ruck=c2=strength=intensity=nutrition="Complete inputs";else{run=b==="RED"?"NO RUN":o==="RED"?"C2 / WALK ONLY":o==="YELLOW"?"Reduce 25–40%; easy":"Full phase plan";ruck=b==="RED"?"NO RUCK":o==="RED"?"No loaded ruck":o==="YELLOW"?"Reduce / light":"Full phase plan";c2=b==="RED"?"Easy C2 if pain-free":o==="RED"?"Recovery C2":o==="YELLOW"?"Prefer easy C2":"Phase plan";strength=b==="RED"?"Non-aggravating only":a==="RED"?"Cut ~50%":o==="YELLOW"?"Cut ~30%":"Full plan";intensity=o==="GREEN"?"Planned":o==="YELLOW"?"Easy only / no hard intervals":"No hard training";nutrition=c==="RED"?"Add energy/carbs; review deficit":c==="YELLOW"?"Hold / add carbs around training":"Follow phase target";if(b==="RED")warn="Mechanical override: do not let favorable HRV justify impact or load carriage.";else if(a==="RED")warn="Systemic recovery flag: reduce stress and reassess."}for(const [id,v] of [["runDecision",run],["ruckDecision",ruck],["c2Decision",c2],["strengthDecision",strength],["intensityDecision",intensity],["nutritionDecision",nutrition]])$(id).textContent=v;$("warningBox").textContent=warn;$("warningBox").classList.toggle("hidden",!warn);$("results").classList.remove("hidden");persistInputs();return{sys:a,mech:b,fuel:c,ov:o,run,ruck,c2}}
function saveLog(){const r=evaluate();if(!r.ov)return;const logs=JSON.parse(localStorage.trainingLogs||"[]");logs.unshift({date:new Date().toISOString(),week:currentWeek(),weight:n("weight"),hrv:n("hrv"),rhr:n("rhr"),sleep:n("sleep"),overall:r.ov,run:r.run,ruck:r.ruck,c2:r.c2});localStorage.trainingLogs=JSON.stringify(logs.slice(0,180));dbSet("trainingLogs",localStorage.trainingLogs);updateDashboard();updateCalibration();renderTrends();$("saveBtn").textContent="Saved ✓";setTimeout(()=>$("saveBtn").textContent="Save Today to Log",1200)}
function renderWeek(){const w=Math.max(1,Math.min(56,parseInt($("weekInput").value||1)));localStorage.programWeek=w;dbSet("programWeek",String(w));const b=blockForWeek(w),days=daysFor(b.phase);$("heroPhase").textContent=`Week ${w} · ${b.phase}`;$("weekSummary").innerHTML=`<div class="week-card"><h3>Week ${w} · ${b.phase}</h3><div class="meta">${w>=53?"TAPER":w%4===0?"CONSOLIDATE WEEK":"BUILD WEEK"}</div><div class="kv"><span>Running target</span><strong>${b.run}</strong><span>Ruck target</span><strong>${b.ruck}</strong><span>Main purpose</span><strong>${b.focus}</strong></div><div class="day-list">${["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((d,i)=>{const det=sessionDetails(days[i]);return `<button class="day day-button" data-session="${encodeURIComponent(days[i])}"><b>${d}</b><div><strong>${det.title}</strong><small>${det.why}</small></div><span>›</span></button>`}).join("")}</div><div id="weekDetail"></div></div>`;document.querySelectorAll(".day-button").forEach(btn=>btn.addEventListener("click",()=>renderWeekDetail(decodeURIComponent(btn.dataset.session))));updateDashboard()}
function renderProgram(){$("programBlocks").innerHTML=blocks.map(x=>`<div class="block-card"><div class="range">WEEKS ${x[1]}–${x[2]}</div><h3>${x[0]}</h3><p>${x[5]}</p><div class="kv"><span>Run</span><strong>${x[3]}</strong><span>Ruck</span><strong>${x[4]}</strong></div></div>`).join("")}
...
document.querySelectorAll(".tab").forEach(btn=>btn.addEventListener("click",()=>{document.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));btn.classList.add("active");document.querySelectorAll(".screen").forEach(x=>x.classList.remove("active"));$(btn.dataset.target).classList.add("active");$("pageTitle").textContent=btn.querySelector("small").textContent;if(btn.dataset.target==="workout")renderWorkout();if(btn.dataset.target==="week")renderWeek();if(btn.dataset.target==="trends")renderTrends()}));
$("evaluateBtn").onclick=evaluate;$("saveBtn").onclick=saveLog;$("completeWorkout").onclick=completeWorkout;$("weekInput").onchange=renderWeek;$("exportBackup").onclick=exportBackup;$("importBackup").onchange=e=>e.target.files[0]&&importBackup(e.target.files[0]).catch(()=>alert("That backup could not be restored."));
["hrv","hrvBase","rhr","rhrBase","sleep","sleepQ","fatigue","grip","gripBase","load","pain","performance","weight","weightAvg"].forEach(id=>{const v=localStorage.getItem("input_"+id);if(v!==null)$(id).value=v});$("focal").checked=localStorage.input_focal==="1";$("gait").checked=localStorage.input_gait==="1";$("weekInput").value=currentWeek();renderWeek();renderProgram();renderWorkout();renderTrends();updateCalibration();requestPersistence();if("serviceWorker"in navigator)addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js"));
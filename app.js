...
function renderProgram(){$("programBlocks").innerHTML=blocks.map(x=>`<div class="block-card"><div class="range">WEEKS ${x[1]}–${x[2]}</div><h3>${x[0]}</h3><p>${x[5]}</p><div class="kv"><span>Running</span><strong>${x[3]}</strong><span>Weighted pack</span><strong>${x[4]}</strong></div></div>`).join("")+`<div class="block-card glossary-card"><div class="range">QUICK TERMS</div><h3>What the training language means</h3><div class="term"><strong>Easy aerobic</strong><p>A pace where breathing stays controlled and you can speak in full sentences.</p></div><div class="term"><strong>Quality run</strong><p>A deliberately faster running session such as controlled intervals or tempo work—not an all-out effort.</p></div><div class="term"><strong>Weighted-pack walk</strong><p>Walking with a loaded backpack. This is often called a “ruck.”</p></div><div class="term"><strong>Threshold</strong><p>A hard but controlled intensity that can be sustained for meaningful work; the app individualizes it rather than assuming one universal heart-rate or lactate number.</p></div><div class="term"><strong>Bodyweight training</strong><p>Push-ups, pull-ups, sit-ups/core work and related movements using your own body weight.</p></div></div>`}

...
must_replace(
    ...
    'function todayKey(){return new Date().toDateString()}function todayCheckin(){return logs().find(x=>new Date(x.date).toDateString()===todayKey())}'
    'function todayWorkout(){return workouts().find(x=>new Date(x.date).toDateString()===todayKey()&&x.completed==="YES")}',
    'function logs(){return JSON.parse(localStorage.trainingLogs||"[]")}function workouts(){return JSON.parse(localStorage.workoutHistory||"[]")}\n'
    'function todayKey(){return new Date().toDateString()}function todayCheckin(){return logs().find(x=>new Date(x.date).toDateString()===todayKey())}'
    'function todayWorkoutRecord(){return workouts().find(x=>new Date(x.date).toDateString()===todayKey())}'
    'function todayWorkout(){const x=todayWorkoutRecord();return x?.completed==="YES"?x:null}',
    "today workout record helper",
)

replace_between(
    "function adaptiveSession(){",
    "function readinessBreakdown(){",
    r'''function adaptiveSessionFor(name,d){
...
function adaptiveSession(){return adaptiveSessionFor(sessionName(),todayCheckin())}''',
    "adaptive session by saved decision",
)

replace_between(
    "function saveWorkout(c){",
    "const EXERCISE_DB_BASE=",
    r'''function saveWorkout(c){
 ...
 $("completionBanner").classList.remove("hidden");setTimeout(()=>$("completionBanner").classList.add("hidden"),1600);renderAll()
    ...
    "workout completion semantics",
)

must_replace(
    'const d=todayCheckin(),det=adaptiveSession(),done=todayWorkout();',
    'const d=todayCheckin(),det=adaptiveSession(),record=todayWorkoutRecord(),done=record?.completed==="YES";',
    "today render workout record",
)
must_replace(
    '$("taskWorkoutSub").textContent=done?"Completed today":det.duration+" · "+det.effort;',
    '$("taskWorkoutSub").textContent=done?"Completed today":record?.completed==="PARTIAL"?"Partial session recorded · review details":record?.completed==="NO"?"Skipped today · review if this changes":det.duration+" · "+det.effort;',
    "today workout status copy",
)
must_replace(
    'const done=todayWorkout();$("completeWorkoutQuick").textContent=done?"✓ Session Complete":"Review & Complete Session";$("completeWorkoutQuick").disabled=!!done;$("postSessionPrompt").classList.toggle("done",!!done);renderLibrary()}',
    'const record=todayWorkoutRecord(),done=record?.completed==="YES";$("completeWorkoutQuick").textContent=done?"✓ Session Complete":record?.completed==="PARTIAL"?"Partial saved · Review":record?.completed==="NO"?"Skipped · Review":"Review & Complete Session";$("completeWorkoutQuick").disabled=!!done;$("postSessionPrompt").classList.toggle("done",!!done);renderLibrary()}',
    "train completion state",
)

must_replace(
    'log.targetCalories=target.cal;log.targetProtein=target.protein;log.targetCarbs=target.carbs;log.targetFat=target.fat;',
    'log.targetCalories=target.cal;log.targetProtein=target.protein;log.targetCarbs=target.carbs;log.targetFat=target.fat;'
    'log.prescribedMeals=meals.map(m=>({...m,foods:[...m.foods]}));log.prescriptionReason=target.adjustment?.copy||target.why;',
    "nutrition prescription history",
)

replace_between(
    "function dateSession(d){",
    "function renderJourney(){",
    r'''function checkinForDate(d){const k=dayKey(d);return logs().find(x=>dayKey(x.date)===k)||null}
...
function renderDaySheetTab(tab){
 ...
 $("daySheetBody").querySelectorAll("[data-dayex]").forEach(b=>b.onclick=()=>openExercise(x.det.steps[+b.dataset.dayex]))
    ...
    "calendar history fidelity",
)

must_replace(
    'const det=makeSession(name),date=dateFor(w,i),is=date.toDateString()===todayKey(),state=dateState(date);',
    'const date=dateFor(w,i),det=dateSession(date).det,is=date.toDateString()===todayKey(),state=dateState(date);',
    "week uses effective prescription",
)
must_replace(
    '(state.failed?"×":state.check&&state.work?"✓":"○")',
    '(state.failed?"×":state.status==="PARTIAL"?"◐":state.status==="NO"?"×":state.check&&state.work?"✓":"○")',
    "week partial skip glyph",
)
